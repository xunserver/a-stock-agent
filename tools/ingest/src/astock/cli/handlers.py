...
def dispatch(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.cmd == "stock" and args.stock_cmd == "news":
        ...
        items = fetch_stock_news(
            code,
            limit=args.limit,
            news_source=registry.news_source(),
        )
    ...
    if args.cmd == "stock" and args.stock_cmd == "events":
        code = args.code.strip().zfill(6)
        items = fetch_stock_events(
            code,
            args.kind,
            limit=args.limit,
            event_source=registry.event_source(),
        )
    ...
    with MarketDB(DB_PATH) as db:
        ...
        if args.cmd == "pool":
            ...
            if args.pool_cmd == "add":
                ...
                print_json(add_index_to_pool(db, args.index, pool_id=args.pool, replace=False, membership_source=registry.membership_source()) if args.index else add_codes_to_pool(db, parse_codes(args.codes), pool_id=args.pool))
                return
            if args.pool_cmd == "set":
                print_json(add_index_to_pool(db, args.index, pool_id=args.pool, replace=True, membership_source=registry.membership_source()))
                return
        ...
        if args.cmd == "stock":
            ...
            if args.stock_cmd == "add":
                ...
                print_json(add_index_to_stocks(db, args.index, membership_source=registry.membership_source()) if args.index else add_codes_to_stocks(db, parse_codes(args.codes)))
                return
            ...
            print_json(sync_stock(
                ...
                profile_source=registry.profile_source(),
                snapshot_source=registry.quote_snapshot_source(),
                valuation_source=registry.valuation_source(),
                fundamental_source=registry.fundamental_source(),
                statement_source=registry.statement_source(),
                bar_source=registry.bar_source(),
                calendar_source=registry.calendar_source(),
            ))
            return
        ...
        if args.cmd == "boards":
            ...
            result = sync_boards(
                ...
                sleep=args.sleep,
                limit=args.limit,
                classification_source=registry.classification_source(),
                membership_source=registry.membership_source(),
            )
